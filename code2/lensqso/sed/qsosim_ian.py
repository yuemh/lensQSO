...
def flatQLF():
    ...
    logPhiStar = -7
    MStar1450 = -27.47 # --> M1450
    MStar = lumfun.PolyEvolParam([MStar1450], z0=6)
    logPhiStar = lumfun.PolyEvolParam([logPhiStar], z0=6)
    alpha = -1
    beta = -1
    return lumfun.DoublePowerLawLF(logPhiStar,MStar,alpha,beta,
                                   cosmo=defaultcosmo)
...
simParams = {
  ...
  'FileName': './simqso_z4.5_6.5_10000deg2',
  ...
  'GridParams':{
    ...
    #'QLFargs':{'skyArea':1e4},       
    'QLFargs':{'skyArea':10000}, # but only 10 deg2 will run a lot faster
...
def main():
    qsoSimulation(simParams, saveSpectra=True)
#    wave,qsos = load_sim_output('highz_qso_sim','.')
