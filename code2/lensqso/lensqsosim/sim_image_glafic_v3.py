...
#        image_dat = self.image_dat
        data = np.loadtxt('/Users/minghao/Research/Projects/lensQSO/exe/out_point.dat')
        image_dat = data[1:].T
    ...
    def sim_lensgal_image_galfit(self):
        dummy = 1

    def sim_image_glafic(self, prefix, x_range, y_range, pix_ext, pix_poi,\
                 psf_file,  **kwargs):
        # define the command list

        command_psf = 'readpsf %s'%psf_file
        command_simimage = 'writeimageall'

        command_list = [command_psf, command_simimage]
...
def main():
    ...
    source_light_host = {'Type':'sersic', 'x_center':0.5, 'y_center':0.4, 'flux':10,\
                  'ellipticity':0.3, 'pa':30, 're':0.7, 'n':1}
    source_light = {'Type':'point', 'x_center':0.5, 'y_center':0.4, 'flux':100}

    lens = OneObject(0.5, mass_component=[lens_sie],\
               light_component=[])
    source = OneObject(5, light_component = [source_light, source_light_host])
...
#    print(lenssystem.image_info(imgsize, pixscale))
    lenssystem.sim_image_glafic('test', [-5, 5], [-5, 5],\
                                pix_ext, pix_poi, psf, 'output.fits')
