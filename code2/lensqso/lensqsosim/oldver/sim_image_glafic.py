...
class LensSystem(object):
    ...
    def plot_lensconfig(self, prefix, output_image, output_fits, \
                       x_range, y_range, psfinfo, pix_ext, pix_poi,\
                        **kwargs):
        imagefile = self.findimage(prefix)
        critfile = self.writecrit(prefix)

        images = np.loadtxt(imagefile, skiprows=1)
        image_x, image_y, image_mu, image_dt = images.T

        critline = np.loadtxt(critfile)

        if not os.path.exists(output_fits):
            self.sim_image(prefix, x_range, y_range, psfinfo,\
                           pix_ext, pix_poi, output_fits, **kwargs)

        image = fits.open(output_fits)[1].data

        fig, ax = plt.subplots(figsize=[5, 5])
        ax.imshow(image, interpolation='none', cmap='gray', origin='lower',\
...
def main():
    ...
    lens_light = {'Type':'sersic', 'flux':1, 'x_center':0., 'y_center':0.,\
                  'ellipticity':0.3, 'pa':30, 're':0.7, 'n':1}
    source_light = {'Type':'point', 'flux':10, 'x_center':0.5, 'y_center':0.4}
...
#    lenssystem.plot_lensconfig(prefix, 'test.pdf','tmp.fits',\
#                               x_range, y_range, psfinfo, pix_ext, pix_poi)
